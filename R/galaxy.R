...
#' `<test>` cases.
#'
#' The tool's requirement is a `<container type="docker">` naming the job's
#' image, so Galaxy runs the job in the same environment as every other
#' target.  Bioconda `<requirement type="package">` entries are not emitted:
#' they would describe a second environment resolved separately from the
#' container, and BiocJobs is not yet on bioconda.
#'
...
galaxyTool <- function(job, pkg_version = NULL, biocjobs_version = NULL,
    ...
    ## Provenance header, matching every other generated artifact.
    xml2::xml_add_child(doc, xml2::xml_comment(sprintf(
        paste0(" Generated by BiocJobs %s from the '%s' job declared in ",
               "Bioconductor package %s (inst/biocjobs/). Do not edit; ",
               "regenerate with: Rscript -e ",
               "'BiocJobs::biocjobsCLI()' galaxy <pkg> %s "),
        biocjobs_version, job$name, job$package, job$name)))
